  ...
  <div className="h-14 border-b border-line bg-ink-800/95 backdrop-blur flex items-center gap-4 px-5 shrink-0 relative z-10">
    ...
    <div className="ml-auto flex items-center gap-3">
      ...
      <div className="flex items-center gap-2 text-[11.5px] text-text-mut pr-3 border-r border-line">
        ...
        <span className="mono">분류: 대내전용</span>
      </div>
      ...
      <div className="flex items-center gap-2 h-8 pl-1 pr-3 rounded-full border border-line bg-ink-700">
        <Avatar name="장" tone="brand" size={22} />
        <div className="leading-tight">
          <div className="text-[11.5px] text-text-hi font-medium">장지훈 경위</div>
          <div className="text-[9.5px] text-text-dim">사이버수사대 · L3</div>
        </div>
      </div>
...
const Sidebar = ({ current, onNavigate }) => {
  const navItems = [
    { key: "chat",   icon: "chat",   label: "AI 어시스턴트",      desc: "채팅 · 에이전트 스토어", badge: 3 },
    { key: "mlops",  icon: "layers", label: "데이터 거버넌스",      desc: "MLOps · 라벨링 · 사전" },
    { key: "llmops", icon: "flask",  label: "프롬프트 실험실",      desc: "LLMOps · A/B" },
    { key: "admin",  icon: "cpu",    label: "자원 관제",          desc: "토큰 · GPU · 신청함",     badge: 7 },
  ];
    ...
    <aside className="w-56 shrink-0 bg-ink-900 border-r border-line flex flex-col">
      ...
      <div className="px-4 pt-4 pb-3 border-b border-line">
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            <div className="text-[13px] font-semibold text-text-hi leading-tight">경찰청 AI 통합</div>
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-3">
        <div className="text-[10px] uppercase tracking-[0.14em] text-text-dim font-semibold px-2 mb-1.5">워크스페이스</div>
        {navItems.map(item => {
        ...
        <div className="text-[10px] uppercase tracking-[0.14em] text-text-dim font-semibold px-2 mt-5 mb-1.5">즐겨찾는 에이전트</div>
        {[
          { name: "@계좌추적",       last: "10:08" },
          { name: "@법령검색",       last: "어제" },
          { name: "@마약동향감지",    last: "03:42", warn: true },
          { name: "@조서요약",       last: "07:14" },
        ].map(a => (
